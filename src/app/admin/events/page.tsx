...
import { CalendarDays, Plus, Pencil, Trash2, X, Save, Loader2, Upload } from "lucide-react";
...
function EventModal({ event, onClose, onSaved }: {
  ...
  const handleImage = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      const url = await uploadImage(file, "aspcs/events");
      ...
      <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }}
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {form.imageUrl && (
              <img src={form.imageUrl} alt="preview" className="mt-2 h-24 w-full rounded-lg object-cover opacity-80" />
            )}
          </div>
          ...
          <label className="flex cursor-pointer items-center gap-3">
            <div onClick={() => setForm((f) => ({ ...f, isHighlight: !f.isHighlight }))}
              className={cn("h-5 w-9 rounded-full transition-colors", form.isHighlight ? "bg-brand-crimson" : "bg-white/10")}>
              <div className={cn("h-5 w-5 rounded-full bg-white shadow transition-transform",
                form.isHighlight ? "translate-x-4" : "translate-x-0")} />
            </div>
...
export default function AdminEventsPage() {
  ...
  const fetchEvents = async () => {
    setLoading(true);
    try {
      const data = await api.get<{ content: Event[] } | Event[]>("/events?size=100&sort=startDate,desc");
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {events.length === 0 && (
            <div className="col-span-full py-16 text-center text-brand-slate">No events yet.</div>
          )}
        </div>
      )}
